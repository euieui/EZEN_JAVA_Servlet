...
public class UploadServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/* 파일 업로드를 위한 준비 */
		// 파일이 업로드 될 타겟 폴더 이름 설정(표면적으로 대표되는 폴더)
		String savePath = "fileUpload"; // 여긴 맵핑
		...
		try {
			MultipartRequest multi = new MultipartRequest(
					request, //*"multipart/form=data" 로 보낸 데이터들은 일반 request 로 전달받을 수 없습니다
					// 따라서 MultiPartRequest 에 request를 넣ㅇ서 복합사용되게 설정합니다
					uploadFilePath, // 서버상의 실제 디렉토리
					uploadFileSizeLimit, // 최대 업로드 파일 크기
					encType, // 인코딩 방법
					new DefaultFileRenamePolicy()
					// 업로드 파일과 동일 이름이 이미 존재하면 새이름 부여 (1,2,3)
			);
			...
			out.print("<br> 글쓴이 : " + name);
			out.print("<br> 제목 : " + title);
