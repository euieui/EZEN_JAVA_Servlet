...
public class MemberServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// command 에 전달된 값이 따라 어떤 작업을 할지가 결정이 되어 실행합니다.
		
		// 각 command 별 실행할 작업들은  해당 실행코드가 들어있는 클래스에 있습니다
		// 클래스 내부중에서도  execute 메서드안에 있습니다.
		...
		// 각 클래스에 있는 execute 메서드는 Action 인터페이스에 존재하는 추상메서드 입니다.
		// 각 클래스가 action 인터페이스를 상속(implement) 하여 ,execute 메서드가 오버라이딩 되면
		// Action 인터페이스의 레퍼런스 변수로
		// 자식클래스의 execute 메서드를 호출하여 사용합니다.
		
		// 1. Action ac = null;
		// 2. if(command.equals("loginForm")) ac = new LoginFormAction();
		// 3. else if (command.equals("joinFor")) ac = new JoinFormAction();
