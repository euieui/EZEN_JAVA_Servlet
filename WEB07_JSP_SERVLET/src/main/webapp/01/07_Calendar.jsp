...
eDay.set(sYear,sMonth+1,1); // 2021 - 11 - 01 (다음달 1일)
// eDay 는 현재 날짜의 다음달 1일로 세팅후 1일을 빼서 이번달 마지막날로 세팅
eDay.add(Calendar.DATE, -1); // 2021 - 10월 마지막 일

// 출력하는 월의 1일자가 무슨 요일인지 정수값으로 계산 1: 일요일, 2: 월요일...
int START_WEEK = sDay.get(Calendar.DAY_OF_WEEK);
int i,k,cnt=0; // i 는 반복실행에 사용, k= 날짜 출력에 사용 cnt= 줄바꿈

%>
<h1><%= sYear %>년 <%=sMonth+1 %>월</h1>
<table width="560" aling="left" cellspacing="1" bgcolor="black">
	<tr bgcolor="white" height="50">
		<td align="center" width="80" style="color:red;">일</td>
		...
		<td align="center" width="80" style="color:blue;">토</td>
	</tr>
	<!-- 첫줄에 나올 날짜들 1일자 요일 전까지 공백, 1, 2, 3..... -->
	<tr bgcolor="white" height="50">
		<% for(i=1; i<START_WEEK; i++) {%>
			<td>&nbsp;</td><!-- 첫칸부터 시작날짜의 요일바로 전열까지는 빈칸 표시 -->
		<%} %>
		<% for(i=1; i<=8-START_WEEK; i++) {%>
			<td align="right"><%=i %>&nbsp;</td> <!-- 1일자부터 토요일까지 날짜 -->
		<%} %>
	</tr>
	<tr bgcolor="white" height="50">
